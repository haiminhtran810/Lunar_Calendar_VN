...
fun universalToJD(dmy: DayMonthYear): Double {
    ...
    if (Y > 1582 || Y == 1582 && M > 10
        ...
        JD = ((367 * Y - INT(
            (7 * (Y + 5001 + INT(((M - 9) / 7).toDouble())) / 4).toDouble()
        )).toDouble()
...
fun can(dmy: DayMonthYear): IntArray {
    ...
    return intArrayOf(
        INT(localToJD(dmy) + 10.5) % 10, (y * 12 + lunar.month + 3) % 10,
        (y + 6) % 10
    )
}
...
fun chi(dmy: DayMonthYear): IntArray {
    ...
    return intArrayOf(
        INT(localToJD(dmy) + 2.5) % 12, (lunar.month + 1) % 12,
        (lunar.year + 8) % 12
    )
}
...
fun saoTot(dmy: DayMonthYear): ArrayList<Int> {
    ...
    val sao = arrayOf(
        intArrayOf(5, 7, 9, 11, 1, 3, 5, 7, 9, 11, 1, 3),
...
fun saoXau(dmy: DayMonthYear): ArrayList<Int> {
    val ret = ArrayList<Int>()
    val saoChi = arrayOf(
        intArrayOf(5, 0, 7, 2, 9, 4, 11, 6, 1, 8, 3, 10),
    ...
    val saoCan = arrayOf(
        intArrayOf(0, 1, 4, 2, 3, 5, 6, 7, 5, 8, 9, 4),
...
fun ngayTot(
    ...
    val arrNgayTot = arrayOf(
        intArrayOf(2, 2), intArrayOf(3, 3), intArrayOf(2, 5), intArrayOf(4, 2),
...
fun cuoiGa(
    dmy1: DayMonthYear,
    dmy2: DayMonthYear, dmyMen: DayMonthYear, dmyWoman: DayMonthYear
): ArrayList<DayMonthYear> {
    ...
    for (i in resultNgayTot.indices) {
        for (j in 0 until counter) {
            if (resultNgayTot[i].day === ret.get(
                    j
                ).day && resultNgayTot[i].month === ret.get(j)
                    .month
            ) {
                k = 0
                break
    ...
    for (i in resultTrucThanh.indices) {
        for (j in 0 until counter) {
            if (resultTrucThanh[i].day === ret.get(
                    j
                ).day && resultTrucThanh[i].month === ret.get(j)
                    .month
            ) {
                k = 0
                break
